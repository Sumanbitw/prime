  ...
  switch (action.type) {
    ...
    case "CREATE__BOOKMARKVIDEOS":
      const savedVideo =
        ...
        action.payload.video.map((videoItem) => {
          return { ...videoItem.video };
        });
      return { ...state, bookmarkVideos: savedVideo };

    case "CREATE__WATCHLATERVIDEOS":
      const savedWatchlaterVideo =
        action.payload.watchlaterVideo &&
        action.payload.watchlaterVideo.map((videoItem) => {
          return { ...videoItem.video };
        });
    ...
    case "CREATE__PLAYLIST":
      ...
      const savedPlaylist =
        action.payload &&
        action.payload.map((videoItem) => {
          return { ...videoItem };
        });
      return { ...state, playlist: savedPlaylist };
    ...
    case "REMOVE__BOOKMARK__VIDEOS":
      return {
        ...state,
        bookmarkVideos: state.bookmarkVideos.filter(
    ...
    case "REMOVE__VIDEOS__FROM__HISTORY":
      return {
        ...state,
        history: state.history.video.filter(
          (videoItem) => videoItem._id !== action.payload._id
        ),
      };
    case "DELETE__PLAYLIST":
      return {
        ...state,
        playlist: [
          ...state.playlist.filter((item) => item?._id !== action.payload),
        ],
      };
      ...
      return {
        ...
        playlist: state.playlist.find((videoItem) =>
          videoItem._id === action.payload.playlistId
            ? {
                ...videoItem,
                videos: videoItem.videos.filter(
