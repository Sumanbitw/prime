
export const data = [
    {   
        id : "10",
        vid:"QD2hxpQd_b0",
        categoryId : "1",
        image:"https://yt3.ggpht.com/ytc/AAUvwnjoPIgYZCMXAkTqrJ84_TQAABqplaHmlfuOMqq0Bw=s88-c-k-c0x00ffffff-no-rj",
        title:"Nidahas Trophy 2018 Final Match, Final Over - India vs Bangladesh",
        description:"Hero NidahasTrophy​ 2018 Final Match, Final Over - India vs Bangladesh at RPICS, Colombo.",
        subscribers: "377", 
        views:"3",
        date:"May 11 2018",
        channelName:"Sri Lanka Cricket",
        category:["batting"]
    } ,
    {
    id :"11", 
    vid:"8oOj2x4_OMs",
    categoryId : "2",
    image:"https://yt3.ggpht.com/ytc/AAUvwnim96pLrDexOYYgsQ2PEU8Q7iz7kPUVF8PDU3pdqQ=s88-c-k-c0x00ffffff-no-rj",
    title:"Batting for Beginners - Cricket Batting Tips",
    description:"Comfortable grip and stance.Watch the ball from bowler's hand.Move back or forward depending on length of each ball.Keep your balance on contact.Keep ball on ground.Play as straight as you can.Look for singles to rotate the strike.",
    subscribers:"377",
    views:"7",
    date:"Aug 28 2020",
    channelName:"Ben Williams - My Cricket Coach",
    category:["batting"],
    },
    
    {
    id:"12",
    vid:"Kk1P_91DaBI",
    categoryId : "3",
    image:"https://yt3.ggpht.com/ytc/AAUvwnjn3qOy6HCjMDhQExDZFX-X2fHSZZOdF0apkIlX=s88-c-k-c0x00ffffff-no-rj",
    title:"Leg Spin Masterclass with Shane Warne and Rashid Khan | The Big Break",
    description:"The greatest leg spinner, Shane Warne, and the best T20 leg spinner, Rashid Khan, grace the MCG to give you a masterclass on leg spin.",
    subscribers:"89.9",
    views:"3",
    date :"Dec 30 2019",
    channelName:"Fox Cricket",
    category:["bowling"],
    }, 
    {
    id:"13",
    vid:"2wdT4zzNinI",
    categoryId:"4",
    image:"https://yt3.ggpht.com/ytc/AAUvwniNv1oOUlrVxCAQQdKIIPf8HC4sG4dAk2ipTVTjFQ=s48-c-k-c0x00ffffff-no-rj",
    title:"Cricket Masterclass: The art of attacking batting with Gilchrist, Pietersen and Ponting",
    description:"Four of the best batsmen of the modern era talk you through the art of attacking batting. Must watch for any cricket fan!",
    subscribers:"3.54",
    views:"9",
    date:"Dec 26 2017",
    channelName:"BT Sport",
    category:["batting"],

    },
    {
    id:"14",    
    vid:"5yTyKmNAxMA",
    categoryId:"5",
    image:"https://yt3.ggpht.com/ytc/AAUvwniL0hCTIn4-UE9SlgiKBZw_uxMs6ys2BQoY6CWF=s88-c-k-c0x00ffffff-no-rj",
    title:"Best Ever Fast Bowling Masterclass With Brett Lee",
    description:"This is the Greatest ever Fast Bowling Masterclass in my opinion in which the Great Australia Fast Bowler Brett Lee talks about How To Bowl outswing, inswing, Bouncer and Yorker. Great Fast Bowling Tips",
    subscribers:"142",
    views:"17",
    date:"Jun 30 2018",
    channelName:"Cricket Masterclass",
    category:["bowling"],

    }
]

export default data

